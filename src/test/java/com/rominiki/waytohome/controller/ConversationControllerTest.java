...
class ConversationControllerTest {
    ...
    @Test
    @WithMockUser(username = "student@test.com", roles = "STUDENT")
    void getMessages_authenticatedUser_returnsMessages() throws Exception {
        ...
        when(chatMessageService.getMessages(100L, "student@test.com"))
                .thenReturn(List.of(message));

        mockMvc.perform(get("/api/conversations/100/messages"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].id").value(500L))
                .andExpect(jsonPath("$[0].conversationId").value(100L))
                .andExpect(jsonPath("$[0].content").value("Hi, is this available?"))
                .andExpect(jsonPath("$[0].read").value(false));

        verify(chatMessageService).getMessages(100L, "student@test.com");
    }
